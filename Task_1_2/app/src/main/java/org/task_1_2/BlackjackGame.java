...
public class BlackjackGame extends Base {
        ...
        if (gameOver == true) {
            ...
            while (!player.isStanding() && !player.isBusted()) {
                ...
                if (input.equalsIgnoreCase("1")) {
                    playerHit();
                    System.out
                            .println("Вы открыли карту "
                                    + player.getPlayerCards().get(player.getPlayerCards().size() - 1) +
                                    "(" + player.getPlayerCards().get(player.getPlayerCards().size() - 1).getIntValue()
                                    + ")");
                    getScore(0);
    ...
    public void playerStand() {
        ...
        System.out.println(
                "Дилер открывает закрытую карту " + dealer.getDealerCards().get(dealer.getDealerCards().size() - 1)
                        + "(" + dealer.getDealerCards().get(dealer.getDealerCards().size() - 1).getIntValue() + ")");
        dealerPlay();
    }

    private void dealerPlay() {
        System.out.println("Карты дилера: " + dealer.getDealerCards() + " => " + dealer.getDealerScore());
        while (dealer.getDealerScore() < 17) {
            dealer.hit();
            System.out.println(
                    "Дилер открывает карту " + dealer.getDealerCards().get(dealer.getDealerCards().size() - 1) + "("
                            + dealer.getDealerCards().get(dealer.getDealerCards().size() - 1).getIntValue() + ")");
            getScore(1);
        }
    }
    ...
    private void getScore(int openDealer) {
        System.out.println("\tВаши карты: " + player.getPlayerCards() + " => " + player.getPlayerScore());
        if (openDealer == 1) {
            System.out.println("\tКарты дилера: " + dealer.getDealerCards() + " => " + dealer.getDealerScore() + "\n");
        } else {
            System.out.println("\tКарты дилера: [" + dealer.getDealerCards().get(0) + ", <закрытая карта>]" + "\n");
        }
    }
