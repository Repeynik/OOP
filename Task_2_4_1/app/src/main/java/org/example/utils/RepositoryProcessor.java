            ...
            for (Student student : group.getStudents()) {
                ...
                try {
                    ...
                    if (taskDirs == null || taskDirs.length == 0) {
                        ...
                        results.add(new StudentResult(student, "None", false, false, false, new TestResults(0, 0, 0), 0,
                                "N/A"));
                        continue;
                    }
                    ...
                    for (File taskDir : taskDirs) {
                        futures.add(executor.submit(() -> {
                            ...
                            if (task == null) {
                                logger.warning("Task " + taskName + " is not recognized in configuration for: "
                                        + student.getFullName());
                            ...
                            try {
                                ...
                                if (!docSuccess) {
                                    logger.warning("Documentation generation failed for task " + taskName + " for: "
                                            + student.getFullName());
                                }
                                ...
                                try {
                                    ...
                                    logger.warning("Style check failed for task " + taskName + " for: "
                                ...
                                try {
                                    for (String weekStart : config.getWeeklyDates()) {
                                        ...
                                        totalActivityScore += ActivityAnalyzer
                                                .analyzeWeeklyActivity(taskDir.getAbsolutePath(), weekStart, weekEnd);
                                    }
                                    lastCommitDate = ActivityAnalyzer.getLastCommitDate(taskDir.getAbsolutePath());
                                } catch (Exception e) {
                                    logger.warning("Error analyzing activity for task " + taskName + " for: "
                                            + student.getFullName());
                                }

                            } catch (Exception e) {
                                logger.severe("Error processing task " + taskName + " for: " + student.getFullName() + ". "
                                        + e.getMessage());
                            }

                            return new StudentResult(student, taskName, buildSuccess, docSuccess, styleSuccess,
                                    testResults, totalActivityScore, lastCommitDate);
                        }));
                    }
